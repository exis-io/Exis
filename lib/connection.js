...
var Domain = function (name) {
   // log.debug("Creating new domain with name: ", name);

   // Last portion of the domain, meant to be somewhat human readable
   // For app level domains it ends up being the fully qualified name
   this.name = name;
   this.domain = name;
...
Domain.prototype.subdomain = function(name) {
   // log.debug("Creating subdomain with name: ", name)

   var domain = new Domain(this.domain + '.' + name)
   domain.superdomain = this;
   domain.name = name;

   this.subdomains.push(domain);

   if (this.joined) {
      // call join on the new domain-- its an instant join
   }
...
Domain.prototype.join = function() {
   ...
   self.connection = new riffle.Connection(self.domain);
    ...
    self.connection.onJoin = function (session) {
        ...
        // Alert all peers of the connection open
        self.joined = true;
...
// Message patterns
Domain.prototype.subscribe = function(endpoint, handler) {
    console.log("Default subscribe");
};

Domain.prototype.register = function(endpoint, handler) {

};

Domain.prototype.call = function(endpoint, args, handler) {

};

Domain.prototype.publish = function(endpoint, args) {

};

Domain.prototype.unsubscribe = function(endpoint, handler) {
    console.log("Default subscribe");
};

Domain.prototype.unregister = function(endpoint, handler) {
