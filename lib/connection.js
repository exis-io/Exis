...
Domain.prototype.subdomain = function(name) {
   ...
   child.superdomain = this;
   this.subdomains.push(child);
...
Domain.prototype.join = function() {
   ...
   self.connection = new riffle.Connection(self.domain);
    ...
    self.connection.onJoin = function (session) {
        ...
        self.handleJoin(self.connection, session);
   };

   self.connection.join();
};

// Handles ivar setup for the domain about to be joined
Domain.prototype.handleJoin = function(connection, session) {
    if (!this.joined) {
        this.joined = true;
    ...
    for (var i = 0; i < this.subdomains.length; i++) {
        console.log("Child: " + this.subdomains[i]);

        if (!this.subdomains[i].joined) {
            this.subdomains[i].handleJoin(connection, session);
        }
    }

    if (this.superdomain && !this.superdomain.joined) {
        this.superdomain.handleJoin();
    }
};
...
Domain.prototype.call = function(action, args, handler) {
    return this.session.call(prependDomain(this.domain, action), args, handler);
};

Domain.prototype.publish = function(action, args) {
