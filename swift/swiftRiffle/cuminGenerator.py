...
outputTemplate = "import Foundation\n\npublic extension Domain {\n"
callerTemplate = '\tpublic func %s<%s>(endpoint: String, _ fn: (%s) -> (%s)) {\n\t\treturn _%s(pdid, fn: cumin(fn))\n\t}'
callTemplate = '\tpublic func %s<%s>(endpoint: String, _ args: AnyObject..., handler fn: ((%s) -> (%s))?) {\n\t\treturn _%s(pdid, args: args, fn: fn == nil ? nil: cumin(fn!))\n\t}'
