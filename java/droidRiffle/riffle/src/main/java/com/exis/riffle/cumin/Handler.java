...
public interface Handler {
    ...
    interface ZeroOne<R> { R run(R r); }

    interface OneOne<A, R> { R run(A a, R r); }

    interface TwoOne<A, B, R> { R run(A a, B b, R r); }

    interface ThreeOne<A, B, C, R> { R run(A a, B b, C c, R r); }

    interface FourOne<A, B, C, D, R> { R run(A a, B b, C c, D d, R r); }

    interface FiveOne<A, B, C, D, E, R> { R run(A a, B b, C c, D d, E e, R r); }

    interface SixOne<A, B, C, D, E, F, R> { R run(A a, B b, C c, D d, E e, F f, R r); }
